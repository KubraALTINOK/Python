...
export default class TemplateSelector {
  ...
  applyGameColorsToSvgMarkup(svgMarkup, gameColors) {
    ...
    return svg.replace(/<svg\b([^>]*)>/i, (match, attrs = "") => {
      ...
      if (styleMatch) {
        ...
        const merged =
          `${existing}${existing.endsWith(";") ? " " : "; "}${colorVars}`.trim();
  ...
  getGameInfo(t) {
    ...
    if (!rec) {
      throw new Error(
        `Template ${t?.id ?? t?.name ?? "unknown"} has invalid gameID: ${t?.gameID}`,
      );
    }
  ...
  buildTemplatePayload(t) {
    ...
    fields.forEach((key) => {
      if (t[key] != null) payload[key] = t[key];
    });
    ...
    if (popupType === "gaming") {
      ...
      if (!gameRecord) {
        throw new Error(
          `Game record not found for template ${t?.id ?? t?.name ?? "unknown"} and gameID ${gameId}`,
        );
      }
      ...
      if (
        Array.isArray(gameRecord.gameRewards) &&
        gameRecord.gameRewards.length
      ) {
        payload.rewards = gameRecord.gameRewards.map((gr) => {
          ...
          return {
            ...
            weight: gr?.weight ?? 0,
            hasReward:
              gr?.hasReward !== undefined ? !!gr.hasReward : code.length > 0,
  ...
  async renderCards(device) {
    ...
    for (let i = 0; i < this.templates.length; i += batchSize) {
      ...
      batch.forEach((t, batchIdx) => {
        ...
        const pinColor1 =
          t.gameColors?.["--pin-color-1"] || readCssVar(svg, "pin-color-1");
        const pinColor2 =
          t.gameColors?.["--pin-color-2"] || readCssVar(svg, "pin-color-2");
        const svgCacheKey = svg ? `${title}|${category}` : "";

        const cardMediaHeight = `height:${device === "desktop" ? "210px" : "300px"};`;

        const cardMediaContent = this.buildGamingMedia(
          t,
          device,
          textColor,
          svg,
        ...
        if (t.categories?.length)
          col.setAttribute("data-categories", t.categories.join(","));
  ...
  renderCachedSvgs() {
    if (!this.grid) return;
    this.grid
      .querySelectorAll(".template-svg-container[data-svg-key]")
      .forEach((holder) => {
  ...
  renderTemplateCategories() {
    ...
    const allBtn = this.categoriesList.querySelector(
      'a[data-category-type="all-templates"]',
    );
    ...
    this.categoriesList
      .querySelectorAll("a[data-category-type]")
      .forEach((a) => {
  ...
  applyAllFilters() {
    ...
    this.grid.querySelectorAll("[data-template-id]").forEach((card) => {
      const name = (
        card.getAttribute("data-template-name") || ""
      ).toLowerCase();
      ...
      const categoryOk =
        !activeCategory || cats.split(",").includes(activeCategory);
  ...
  updateEmptyState() {
    ...
    const visible = Array.from(
      this.grid.querySelectorAll("[data-template-id]"),
    ).filter(
      (c) => !c.classList.contains("d-none") && c.style.display !== "none",
    ).length;
  ...
  async init() {
    const section = document.querySelector(
      "section.section[data-promotion-name]",
    );
    if (!section) return;

    this.promotionName = section.getAttribute("data-promotion-name") || "";
    this.promotionWebsite =
      section.getAttribute("data-promotion-website") || "";
    ...
    const initialDevice = window.matchMedia?.("(max-width: 576px)")?.matches
      ? "mobile"
      : "desktop";
    ...
    this.platformTypeSelect?.addEventListener("change", (e) =>
      this.setActiveDevice(e.target.value),
    );

    this.searchInput?.addEventListener("input", () => {
      if (this.searchInput.value.trim() && this.categoriesList) {
        const links = this.categoriesList.querySelectorAll(
          "a[data-category-type]",
        );
        const allBtn = this.categoriesList.querySelector(
          'a[data-category-type="all-templates"]',
        );
