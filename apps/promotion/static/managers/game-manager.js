...
import { initScratchCard } from "../scratch/scratch.js";
...
function _resolveGameSvgRoot(theme) {
  ...
  if (tr) return tr;
  return String(
    getTemplateById(theme?.template)?.game_styles?.game_svg ?? "",
  ).trim();
}
...
function _getGameSvgRootFontScale(svg) {
  if (!svg || typeof svg.viewBox === "undefined" || !svg.viewBox.baseVal)
    return 1;
...
function _scaleGameSvgTextCssForLargeViewBox(styleText, scale) {
  if (!styleText || scale === 1 || !Number.isFinite(scale) || scale <= 0)
    return styleText;
...
export function truncateRewardLabel(raw) {
  ...
  if (!s) return "";
  return s.length <= REWARD_LABEL_MAX_LENGTH
    ? s
    : s.slice(0, REWARD_LABEL_MAX_LENGTH);
}
...
export class GameManager {
  ...
  getCurrentGameType() {
    if (
      ...
      (!this.theme || !getGameTypeName(this.theme))
    ) {
      return "";
    }
    ...
    if (
      this._isUnifiedShell() &&
      (!this.currentView || this.currentView.trim() === "")
    ) {
      return "";
    }
  ...
  _gameColorsFromLinkedThemes() {
    return (
      this.theme?.gameColors || this.themeManager?.theme?.gameColors || null
    );
  }
  ...
  _validateGameType(gameType) {
    ...
    if (/^\d+$/.test(compact)) {
      ...
      const name =
        rec?.name != null ? String(rec.name).toLowerCase().trim() : "";
  ...
  _updateGameColorLabels() {
    ...
    if (!raw) {
      this._applyGameColorLabels(
        this._isUnifiedShell()
          ? GAME_COLOR_LABELS._empty
          : GAME_COLOR_LABELS.wheel,
      );
      return;
    }
  ...
  _openRewardsAccordion() {
    ...
    const accordionButton =
      ...
      document.querySelector(
        '.accordion-button[aria-controls="collapseRewards"]',
      );
  ...
  _renderGameRewards(gameRewards) {
    ...
    gameRewards.forEach((reward, index) => {
      ...
      toggleInput.addEventListener("change", () => {
        const rewardsContainer = document.getElementById(
  ...
  _updateRewardsVisibility() {
    ...
    const container = document.getElementById("gameRewardsContainer");
    const hasRows =
      container && container.querySelectorAll(".reward-row").length > 0;
  ...
  _randomizeWeights() {
    ...
    activeIndices.forEach((rowIdx, i) => {
      const weightInput = container.querySelector(
        `input[name="reward_weight_${rowIdx}"]`,
      );
      if (weightInput) {
  ...
  _loadInitialRewards() {
    ...
    if (
      gameData &&
      gameData.gameRewards &&
      Array.isArray(gameData.gameRewards)
    ) {
  ...
  _openChangeGameAccordion() {
    ...
    const accordionButton =
      ...
      document.querySelector(
        '.accordion-button[aria-controls="collapseGameType"]',
      );
  ...
  _bindGamePreviewClickOpensAccordion() {
    if (typeof document === "undefined") return;
    if (document.documentElement.dataset.gamePreviewAccordionBound === "1")
      return;
    ...
    document.body.addEventListener("click", (e) => {
      ...
      if (
        !accordionItem ||
        window.getComputedStyle(accordionItem).display === "none"
      )
        return;
  ...
  _setupGamePositionRadios() {
    if (typeof document === "undefined") return;
    if (document.documentElement.dataset.gamePositionRadiosBound === "1")
      return;
  ...
  _readGamePositionFromForm() {
    const el = document.querySelector(
      'input[name="game_position"]:not(:disabled):checked',
    );
  ...
  _commitGamePositionFromForm() {
    ...
    if (forTheme !== "left" && forTheme !== "right" && forTheme !== "top")
      return;
    ...
    if (
      this.themeManager &&
      typeof this.themeManager.renderPromotionPreview === "function"
    ) {
  ...
  _coerceGamePositionForLightSplitLayout(pos) {
    ...
    const lt = String(activeTheme?.layout?.type || "")
      .toLowerCase()
      .trim();
  ...
  _syncGamePositionUi() {
    ...
    const layoutType = String(activeTheme?.layout?.type || "")
      .toLowerCase()
      .trim();
    const splitRow =
      this._isUnifiedShell() && _isSplitRowLayoutType(layoutType);
    ...
    if (vWrap)
      vWrap.style.display = !splitRow && orient === "vertical" ? "" : "none";
    ...
    const idByPos =
      orient === "horizontal"
        ...
        : {
            left: "game_pos_v_left",
            top: "game_pos_v_top",
            right: "game_pos_v_right",
          };
  ...
  _shellWidthGameLikePositionForLightDecor(activeTheme) {
    if (
      !activeTheme ||
      String(activeTheme.popup_type || "").toLowerCase() !== "gaming"
    )
      return null;
    const decorPath = String(
      activeTheme?.image?.path || activeTheme?.top_image?.path || "",
    ).trim();
    if (!decorPath) return null;

    const imgPos = String(
      activeTheme?.image?.position || activeTheme?.top_image?.position || "",
    )
      .trim()
      .toLowerCase();
  ...
  applyGamePositionLayout() {
    ...
    const shell =
      typeof document !== "undefined"
        ? document.getElementById("wheelluckContainer")
        : null;
    if (!shell) return;
    if (gid == null) {
      const suppress = Boolean(
        this.themeManager?._suppressNoGameLayoutMutations,
      );
      if (!suppress && !shell.classList?.contains("mobile-preview-mode")) {
        if (
          typeof window !== "undefined" &&
          window.wheelApp?.imageManager
      ...
      if (!suppress) {
        const sideForShell =
          this._shellWidthGameLikePositionForLightDecor(activeTheme);
        this._applyDynamicShellWidthForGame(sideForShell, shell);
      }
      return;
    }
    const isMobilePreview = Boolean(
    ...
    this._applyGameStyleBucketForPosition(
      this._resolveGameStylesLayoutPosition(),
    );
    this._syncShellGamePositionAttribute(
      this._resolveGameStylesLayoutPosition(),
    );
  }
  ...
  _applyDynamicShellWidthForGame(position, shell) {
    ...
    const activeTheme = this._activeTheme();
    const tplKey =
      activeTheme?.template != null ? Number(activeTheme.template) : NaN;
    const legacyId = Number(activeTheme?.id);
    const templateId =
      Number.isInteger(tplKey) && tplKey > 0
        ? tplKey
        : Number.isInteger(legacyId) && legacyId > 0
          ? legacyId
          : NaN;
    const template = !Number.isNaN(templateId)
      ? getTemplateById(templateId)
      : null;
    const cs =
      activeTheme?.containerStyle &&
      typeof activeTheme.containerStyle === "object"
        ? activeTheme.containerStyle
        : {};
    const tplCs =
      template?.containerStyle && typeof template.containerStyle === "object"
  ...
  _applyGameStyleBucketForPosition(position) {
    ...
    if (gameContainer && g) {
      ...
      applyStyleString(
        gameContainer,
        stripped && stripped.trim() ? stripped : g,
      );
    }
  ...
  _resetContentWrapperForNoGameEditor(shellEl) {
    ...
    if (!splitRow && !noSideImage) {
      const pos = _normalizeGameLayoutPos(
        _resolveLayoutPositionFromType(activeTheme?.layout || {}),
      );
      this._applyShellStyleBucketsForPosition(pos);
    ...
    if (splitRow && !noSideImage) {
      let pos = String(activeTheme?.layout?.position || "")
        .trim()
        .toLowerCase();
      ...
      const im =
        typeof window !== "undefined" ? window.wheelApp?.imageManager : null;
      if (im && typeof im.applyLightSplitColumnOrder === "function") {
        ...
        if (
          pos === "top" &&
          typeof im.applyPositionStyleBuckets === "function"
        ) {
  ...
  _applyTemplateDefaultsForUnifiedGameRestart() {
    ...
    const tid =
      typeof rawTid === "number" ? rawTid : parseInt(String(rawTid ?? ""), 10);
    const tpl = Number.isInteger(tid) && tid > 0 ? getTemplateById(tid) : null;
    if (!tpl) return;

    const layout =
      tpl.layout && typeof tpl.layout === "object" ? tpl.layout : {};
    const posRaw = String(layout.position ?? "left")
      .trim()
      .toLowerCase();
    const pos = ["left", "right", "top"].includes(posRaw) ? posRaw : "left";
    const tplGs =
      tpl.game_styles && typeof tpl.game_styles === "object"
        ? tpl.game_styles
        : null;
    ...
    this._forEachLinkedTheme((t) => {
      ...
      if (tplGs) {
        t.game_styles =
          t.game_styles && typeof t.game_styles === "object"
            ? t.game_styles
            : {};
    ...
    const heightIn =
      this.lightPopupGameSvgHeightInput ||
      document.getElementById("lightPopupGameSvgHeight");
    const sizeVal =
      this.lightPopupGameSvgSizeValue ||
      document.getElementById("lightPopupGameSvgSizeValue");
  ...
  changeGameType(gameType, options = {}) {
    ...
    if (
      this.themeManager &&
      this.themeManager.popupManager &&
      typeof this.themeManager.popupManager.updateCirclePinVisibility ===
        "function"
    ) {
    ...
    const selectedGameId =
      Number.isInteger(rawGameId) && rawGameId > 0 ? rawGameId : null;
    const gameData =
      selectedGameId != null
    ...
    if (this.themeManager?.colorManager) {
      setTimeout(() => {
        if (preferredColors && Object.keys(preferredColors).length > 0) {
          this.themeManager.colorManager.applyGameColorsFromTemplate(
            preferredColors,
          );
        } else {
        ...
        if (
          typeof this.themeManager
            ._updateGameColorsResetBaselineFromCurrentTheme === "function"
        ) {
    ...
    if (!skipPreviewUpdate) {
      ...
      const updatedInPlace = this._replaceCurrentGameSvgMarkup(
        svgMarkup,
        validatedType,
      );
    ...
    if (gameTypeChanged && this._isUnifiedShell()) {
      ...
      if (
        this.themeManager &&
        typeof this.themeManager._resetGameAreaBackgroundFromTemplate ===
          "function"
      ) {
  ...
  _resolveGamePalette(gameData, options = {}) {
    ...
    if (
      themeStored &&
      typeof themeStored === "object" &&
      Object.keys(themeStored).length > 0
    ) {
      return { ...themeStored };
    }
    ...
    const tid =
      ...
      this.theme?.template ??
      this.themeManager?.theme?.template;
  ...
  _replaceCurrentGameSvgMarkup(svgMarkup, gameType) {
    ...
    const sizePercent =
      this._extractHeightPercent(getGameHeightStyle(this.theme)) ||
      this.gameSvgDefaultHeight;
    this._applyGameSvgSizeWithContainer(
      sizePercent,
  ...
  _getOrCreateGameInnerForSvgSwap() {
    let gameInner = document.querySelector(
      ".game-svg-container .game-svg-inner",
    );
    if (gameInner) return gameInner;

    const content = document.getElementById("wheelluckContent");
    const contentWrapper =
      document.querySelector(".contentWrapper") || content?.parentElement;
  ...
  _syncDefaultGameHeightFromTheme() {
    const configured = this._extractHeightPercent(
      getGameHeightStyle(this.theme),
    );
  ...
  initShellGameControlsDOM() {
    ...
    this.gameTextFontSizeInput = document.getElementById(
      "gameTextFontSizeInput",
    );
    this.gameTextFontRow = document.getElementById("gameTextFontRow");
    this.lightPopupGameSvgHeightInput = document.getElementById(
      "lightPopupGameSvgHeight",
    );
    this.lightPopupGameSvgSizeValue = document.getElementById(
      "lightPopupGameSvgSizeValue",
    );
    this.removeGameButton = document.getElementById("removeGameButton");
    this.removeGameButtonWrapper = document.getElementById(
      "removeGameButtonWrapper",
    );
  ...
  setInitialGameSvg() {
    ...
    if (!this._isUnifiedShell()) {
      if (this.gameSvgSizeControls)
        this.gameSvgSizeControls.style.display = "none";
      if (this.removeGameButtonWrapper)
        this.removeGameButtonWrapper.style.display = "none";
    } else {
      this._syncShellGameAuxUiVisibility();
    }

    if (this._isUnifiedShell()) {
      const template = getTemplateById(this.theme.template) || {};
      ...
      if (hasGameEnabled) {
        ...
        if (!gameType) {
          if (hasActiveLightGame(this.theme)) {
            ...
            const rawFallbackId = Number(
              template.gameID ?? template.defaultGame,
            );
            const fallbackId =
              Number.isInteger(rawFallbackId) && rawFallbackId > 0
                ? rawFallbackId
                : null;
            if (fallbackId != null) {
      ...
      if (isGaming && hasGameEnabled && !gameType) {
        ...
        gameType = (getGameTypeName(this.theme) || "").trim();
        if (!gameType) {
          gameType = this._validateGameType(
            templateDefaultGameToTypeName(template) || "wheel",
          );
        }
      }
  ...
  restoreUnifiedLightGameAfterTopImageCleared() {
    ...
    if (
      tm?.templateManager &&
      typeof tm.templateManager._applyTemplateGameDefaults === "function"
    ) {
  ...
  applyGameSvgWithType(gameType, skipPreviewUpdate = false) {
    ...
    contentWrapper
      .querySelectorAll(".wheelContainer")
      .forEach((n) => n.remove());
    ...
    if (!gameContainer) {
      ...
      contentWrapper.insertBefore(
        gameContainer,
        wheelPopupContent || contentWrapper.firstChild,
      );
    }
    ...
    if (gameType === "scratchcard") {
      setTimeout(() => {
        initScratchCard();
      }, 100); // Kutu ekrana tam çizilsin diye saniyenin onda biri kadar beklettim
    }
    // ---------------------------------------------------
  }
  ...
  _fitShellGameSvgElements(wrapperEl) {
    ...
    const targets =
      directRoots.length > 0
        ? directRoots
        : [wrapperEl.querySelector("svg")].filter(Boolean);
    targets.forEach((svg) => {
  ...
  _applyGameSvgSizeWithContainer(sizePercent, container) {
    const gameContainer =
      container || document.querySelector(".game-svg-container");
    ...
    const isLightPopup =
      String(activeTheme?.popup_type || "")
        .toLowerCase()
        .trim() === "gaming";
    const isLightSplit =
      this._isUnifiedShell() && (layoutPos === "left" || layoutPos === "right");
    ...
    const baseInner = _resolveGameBucketField(
      activeTheme,
      layoutPos,
      "game_inner",
    );
        ...
        const popupH = Math.max(
          ...
          Math.round(
            rectH ||
              ...
              parseCssPx(
                containerStyleValue(this.theme?.containerStyle, "height"),
              ) ||
              450,
          ),
        );
    ...
    const containerStyleStr = usePercentGameHeight
      ...
      : isLightTopShell
        ...
        : [baseOuterSansBg, `height: ${containerHeightPx}px`]
            .filter(Boolean)
            .join("; ");
    ...
    if (isLightSplit) {
      ...
      if (inner) {
        applyStyleString(
          inner,
          [baseInner, areaStyle].filter(Boolean).join("; "),
        );
        this._fitShellGameSvgElements(inner);
        this._applyGameSvgTextStylesToContainer(inner);
      }
  ...
  syncAfterNoGameTemplate() {
    ...
    if (
      this.gameTypeSelect &&
      typeof this._setupGameTypeSelect === "function"
    ) {
      this._setupGameTypeSelect();
    }
    this.applyGamePositionLayout();
  ...
  removeGameSvg() {
    ...
    document
      .querySelector("#wheelluckContainer")
      ?.classList.remove("has-game-svg");
    this.applyGamePositionLayout();
    this._syncGamePositionUi();
    this.updateGameControlsVisibility();
    this.themeManager?.scheduleDesktopPreviewFit?.();
  ...
  syncGameTextFontSelectFromTheme() {
    ...
    if (this.gameTextFontSizeInput) {
      ...
      this.gameTextFontSizeInput.value = Number.isFinite(px)
        ? String(px)
        : "28";
    }
  }
  ...
  _commitGameSvgTextStyleFromControls() {
    ...
    if (!Number.isFinite(px)) {
      px = parseInt(
        parseGameSvgTextFontSize(this.theme?.game_styles?.game_svg_text),
        10,
      );
    }
    ...
    if (this.gameTextFontSizeInput)
      this.gameTextFontSizeInput.value = String(px);
    const payload = { fontFamily: label, fontSize: `${px}px` };
    this._forEachLinkedTheme((t) => {
      t.game_styles =
        t.game_styles && typeof t.game_styles === "object" ? t.game_styles : {};
  ...
  _updateSliceColorsVisibility() {
    ...
    const hasGame =
      this.gameTypeSelect.value.trim() || getGameTypeName(this.theme);
  ...
  updateGameControlsVisibility() {
    if (!isUnifiedPopupShellTheme(this.theme)) {
      if (this.changeGameAccordion)
        this.changeGameAccordion.style.display = "none";
      if (this.gameSvgSizeControls)
        this.gameSvgSizeControls.style.setProperty(
          "display",
          "none",
          "important",
        );
      if (this.removeGameButtonWrapper)
        this.removeGameButtonWrapper.style.setProperty(
          "display",
          "none",
          "important",
        );
    ...
    if (this.changeGameAccordion)
      this.changeGameAccordion.style.display = "block";
  ...
  setupGameEventListeners() {
    ...
    if (
      this.gameTextFontSelect &&
      this.gameTextFontSelect.dataset.fontChangeBound !== "1"
    ) {
      this.gameTextFontSelect.dataset.fontChangeBound = "1";
      this.gameTextFontSelect.addEventListener("change", () =>
        this._commitGameSvgTextStyleFromControls(),
      );
    }
    if (
      this.gameTextFontSizeInput &&
      this.gameTextFontSizeInput.dataset.gameTextSizeBound !== "1"
    ) {
      this.gameTextFontSizeInput.dataset.gameTextSizeBound = "1";
      this.gameTextFontSizeInput.addEventListener("input", () =>
        this._commitGameSvgTextStyleFromControls(),
      );
    }
    ...
    this.gameTypeSelect.addEventListener("change", (event) => {
      ...
      this.changeGameType(newGameType, {
        skipPreviewUpdate: false,
        updateDOM: true,
      });
      this._updateSliceColorsVisibility();
